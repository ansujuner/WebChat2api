...
export function TrayView() {
  ...
  useEffect(() => {
    loadData()
    return window.electronAPI?.accounts?.onChanged(() => { void loadData() })
  }, [])
  ...
  const handleCopyUrl = async () => {
    if (port === null) return
    const url = localProxyOrigin({ host, port })
  ...
  const toggleProxy = async () => {
    ...
    try {
      ...
      setPort(status.port)
      setHost(status.host || '127.0.0.1')
      if (!success) console.error('Proxy start or stop failed; see application Logs')
    } catch (error) {
      console.error('Failed to change proxy state:', error)
    } finally {
  ...
  const providerList: ProviderInfo[] = useMemo(() => {
    return providers.map((p) => {
      ...
      return {
        ...
        activeCount: providerAccounts.filter((a) => accountAvailability(a).available).length,
      }
    })
  }, [providers, accounts])
      ...
      <section className="flex-none px-4 -mt-3 relative z-10 no-drag">
        <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-2xl shadow-lg border border-white/50 dark:border-slate-700/50 p-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              onClick={toggleProxy}
              disabled={proxyLoading || port === null}
              className={cn(
        ...
        {isLoading && selectedProviderAccounts.length === 0 ? (
          ...
          <div className="space-y-2 pb-3">
            {selectedProviderAccounts.map((account) => {
              const isActive = accountAvailability(account).available
              return (
                <div
                  key={account.id}
                  className={cn(
