...
const ModelRow = memo(({ model, isSelected, onSelect, t }: ModelRowProps) => {
  const providerIcon = model.providerId === 'mapping' ? modelMappingIcon : providerIcons[model.providerId]
  ...
  return (
    ...
    >
      ...
      <TableCell>
        ...
        {model.label && <div className="mt-1 text-xs text-muted-foreground">{model.label}</div>}
      </TableCell>
      <TableCell>
...
export function ModelList() {
  ...
  const modelList = useMemo(() => {
    ...
    for (const provider of enabledProviders) {
      ...
      const hasActiveAccounts = providerAccounts.some(a => accountAvailability(a).available)
    ...
    for (const provider of enabledProviders) {
      ...
      for (const model of effectiveModels) {
        ...
        models.push({
          ...
          label: provider.id === 'deepseek' && !model.isCustom
            ? t(`deepseek.models.${model.displayName}`, { defaultValue: '' })
            : undefined,
