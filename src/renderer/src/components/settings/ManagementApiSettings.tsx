...
export function ManagementApiSettings() {
  ...
  useEffect(() => {
    loadConfig()
    const observer = observeProxyStatus(window.electronAPI, setProxyStatus, error => console.error('Failed to read proxy status:', error))
  ...
  const apiEndpoint = proxyStatus ? `${localProxyOrigin(proxyStatus)}/v0/management` : '—'
