...
import type { VerifiedAccountInfo } from '../../../../shared/accountIdentity'
...
interface AddProviderDialogProps {
  ...
  onSelectBuiltin: (provider: BuiltinProviderConfig, credentials: Record<string, string>, accountInfo?: VerifiedAccountInfo) => Promise<void>
  onCreateCustom: () => void
    ...
    if (fieldName) {
      ...
      if (providerId === 'deepseek' && tokenValue && tokenValue.startsWith('{') && tokenValue.endsWith('}')) {
        try {
          ...
          console.error('[mapOAuthCredentials] Invalid JSON token wrapper')
        }
      }
...
export function AddProviderDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onSelectBuiltin(selectedProviderData, credentials, validationResult.valid ? validationResult.userInfo : undefined)
      onOpenChange(false)
      setStep(1)
      setSelectedProvider(null)
      setSearchQuery('')
  ...
  const handleNextStep = () => {
    if (selectedProvider) {
      setActiveTab(selectedProvider === 'arena' ? 'oauth' : 'manual')
      setStep(2)
    }
  }
  ...
  const renderStep1 = () => (
    <Tabs value={providerTab} onValueChange={setProviderTab} className="mt-4">
      ...
      <div className="border-t pt-4">
        ...
        {supportsOAuth ? (
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className={selectedProviderData?.id === 'arena' ? 'grid w-full grid-cols-1' : 'grid w-full grid-cols-2'}>
              {selectedProviderData?.id !== 'arena' && <TabsTrigger value="manual">{t('providers.manualInput')}</TabsTrigger>}
              <TabsTrigger value="oauth">{t(selectedProviderData?.id === 'arena' ? 'arena.browserLogin' : 'providers.oauthLogin')}</TabsTrigger>
            </TabsList>
              ...
              <div className="flex flex-col items-center justify-center py-6 space-y-4">
                <div className="text-center">
                  <p className="text-sm text-muted-foreground mb-4">
                    {t(selectedProviderData?.id === 'arena' ? 'arena.browserLoginHelp' : selectedProviderData?.id === 'deepseek' ? 'deepseek.externalBrowserLoginHelp' : 'providers.clickToOpenOAuth')}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {t(selectedProviderData?.id === 'arena' ? 'arena.profileOnlyHelp' : 'providers.oauthAutoCapture')}
                  </p>
                </div>
                <Button 
                ...
                >
                  {isOAuthLoading ? (
                    ...
                    <>
                      ...
                      {t(selectedProviderData?.id === 'arena' ? 'arena.browserLogin' : 'providers.openOAuthLogin')}
                    </>
                  )}
                </Button>
          ...
          {step === 1 ? (
            ...
            <>
              ...
              <Button
                ...
                disabled={isValidating || isSubmitting || (selectedProviderData?.id === 'arena' && !validationResult.valid)}
              >
                {isSubmitting ? (
                  <>
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
