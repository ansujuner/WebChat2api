...
export function AccountList({
  ...
  const activeCount = accounts.filter(a => accountAvailability(a).available).length
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="font-medium truncate" title={account.name}>{account.name}</span>
                          <span className="text-xs text-muted-foreground font-mono shrink-0" title={`ID: ${account.id}`}>#{shortAccountId(account.id)}</span>
                          <Badge 
                            variant="outline" 
                            className={cn('text-xs', config.color, config.bgColor)}
                          >
                            <StatusIcon className="mr-1 h-3 w-3" />
                            {t(account.status === 'active' && !accountAvailability(account).available ? 'accountAvailability.credentialValid' : config.labelKey)}
                          </Badge>
                        </div>
