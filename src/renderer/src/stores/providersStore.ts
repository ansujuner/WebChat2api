...
export const useProvidersStore = create<ProviderState>((set, get) => ({
  ...
  setAccounts: (accounts) => set(state => ({ accounts, accountCounts: Object.fromEntries(state.providers.map(provider => {
    const selected = accounts.filter(account => account.providerId === provider.id)
    return [provider.id, { total: selected.length, active: selected.filter(account => accountAvailability(account).available).length }]
  })) })),
