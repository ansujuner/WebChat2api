...
interface ToolCallingAPI {
  getStatus: () => Promise<{ models: Array<{model:string;providerId:string;providerName:string}> }>
  runSmoke: (input: { clientAdapterId?: string; model?: string; providerId?: string }) => Promise<{ success: boolean; message?: string; category?: string; model?: string; providerId?:string }>
}
