...
const accountsAPI = {
  ...
  clearSuspension: (id: string): Promise<Account | null> =>
    ipcRenderer.invoke(IpcChannels.ACCOUNTS_CLEAR_SUSPENSION, id),
...
const appAPI = {
  ...
  getRuntimeInfo: (): Promise<BrowserRuntimeInfo> =>
    ipcRenderer.invoke(IpcChannels.APP_GET_RUNTIME_INFO),
...
const toolCallingAPI = {
  getStatus: () => ipcRenderer.invoke(IpcChannels.TOOL_CALLING_GET_STATUS),
  runSmoke: (input: { clientAdapterId?: string; model?: string; providerId?: string }) =>
    ipcRenderer.invoke(IpcChannels.TOOL_CALLING_RUN_SMOKE, input),
}
const trayAPI = {
  openDashboard: (): void => 
