...
export interface Account {
  ...
  /** Account name */
  name: string
  /** Automatic identity label or an explicit user-defined label. */
  nameSource?: 'auto' | 'custom'
  /** Account email (optional) */
  email?: string
  /** Public identifier returned by a successful provider validation. */
  ...
  enabled?: boolean
  /** Temporary suspension expiry in epoch milliseconds; no expiry requires explicit manual release. */
...
export interface Provider {
  ...
  /** Fields required by custom and built-in account forms. */
  credentialFields?: CredentialField[]
  /** Provider status */
...
export interface AppConfig {
  ...
  /** Outbound login/validation/chat proxy (legacy key): system follows OS/PAC; none connects directly. */
...
export const DEEPSEEK_PRIMARY_MODELS = ['deepseek-v4-flash', 'deepseek-v4-pro', 'deepseek-v4-flash-vision-exp']
...
// Migration evidence only. These convenience names must never be seeded again.
export const LEGACY_DEEPSEEK_AUTO_MODEL_MAPPINGS: Record<string, ModelMapping> = {
...
export function normalizeModelMappingsWithDefaults(
  ...
  const customMappings = Object.fromEntries(
    Object.entries(mappings || {}).filter(([requestModel, mapping]) => !isLegacyDeepSeekAutoMapping(requestModel, mapping))
      .map(([requestModel, mapping]) => [requestModel, { ...mapping }]),
  )
...
/** Keep deliberate retargeting/account bindings and aliases owned by other providers. */
function isLegacyDeepSeekAutoMapping(requestModel: string, mapping: ModelMapping): boolean {
  if (!mapping || mapping.requestModel !== requestModel || mapping.preferredAccountId
    || (mapping.preferredProviderId && mapping.preferredProviderId !== 'deepseek')) return false
  const oldDefault = LEGACY_DEEPSEEK_AUTO_MODEL_MAPPINGS[requestModel]
  if (oldDefault) return mapping.actualModel === oldDefault.actualModel && mapping.preferredProviderId === 'deepseek'
  return DEEPSEEK_LEGACY_MODEL_MAPPING_NAMES.includes(requestModel)
    && ['deepseek-chat', 'deepseek-reasoner', 'deepseek-v4-flash'].includes(mapping.actualModel)
}

function isLegacyDeepSeekOverride(displayName: string, actualModelId: string): boolean {
  if (DEEPSEEK_PRIMARY_MODELS.includes(displayName)) return displayName === actualModelId
  const oldDefault = LEGACY_DEEPSEEK_AUTO_MODEL_MAPPINGS[displayName]
  if (oldDefault) return actualModelId === oldDefault.actualModel
  return DEEPSEEK_LEGACY_MODEL_MAPPING_NAMES.includes(displayName)
    && ['deepseek-chat', 'deepseek-reasoner', 'deepseek-v4-flash'].includes(actualModelId)
}
...
export function sanitizeDeepSeekModelOverrides(
  ...
  const obsoleteNames = new Set([
    ...
    ...Object.keys(LEGACY_DEEPSEEK_AUTO_MODEL_MAPPINGS),
  ])
  const addedModels = (overrides?.addedModels || [])
    .filter(model => !isLegacyDeepSeekOverride(model.displayName, model.actualModelId))
    .map(model => ({ ...model }))

  return {
    addedModels,
    excludedModels: (overrides?.excludedModels || []).filter(model =>
      !obsoleteNames.has(model) || addedModels.some(custom => custom.displayName === model)
    ),
  }
}
