...
export class StoreManager {
  ...
  private accountCooldownTimer?: ReturnType<typeof setTimeout>
      ...
      if (p.type === 'builtin') {
        ...
        if (builtinConfig) {
          ...
          return { 
            ...
            ...(p.id === 'arena' ? savedArenaCatalog(p.supportedModels, p.modelMappings) : {
  ...
  /** Expire only scheduling holds, never authentication failures or the manual switch. */
  refreshAccountAvailability(): void {
    if (!this.store) return
    const now = Date.now()
    const accounts = (this.store.get('accounts') || []) as Account[]
    let changed = false
    const next = accounts.map(account => {
      if (account.cooldownUntil === undefined || !Number.isSafeInteger(account.cooldownUntil) || account.cooldownUntil > now) return account
      const { cooldownUntil: _until, cooldownReason: _reason, ...rest } = account
      changed = true
      return { ...rest, updatedAt: now }
    })
    if (changed) { this.store.set('accounts', next); this.notifyAccountsChanged() }
    if (this.accountCooldownTimer) clearTimeout(this.accountCooldownTimer)
    this.accountCooldownTimer = undefined
    const deadlines = next.map(account => account.cooldownUntil).filter((until): until is number => Number.isSafeInteger(until) && until! > now)
    if (deadlines.length) {
      this.accountCooldownTimer = setTimeout(() => {
        try { this.refreshAccountAvailability() } catch { console.error('[Store] Failed to refresh account scheduling state') }
      }, Math.min(2147483647, Math.max(1, Math.min(...deadlines) - now)))
      this.accountCooldownTimer.unref()
    }
  }

  private notifyAccountsChanged(): void {
    // Payload deliberately contains no account identity or credentials; renderers fetch their masked view.
    const windows = BrowserWindow?.getAllWindows?.() || (this.mainWindow ? [this.mainWindow] : [])
    for (const window of windows) {
      if (!window.isDestroyed()) window.webContents.send('accounts:changed')
    }
  }

  /** Migrate only known generated labels; never decrypt credentials during migration. */
  private initializeAccountNames(): void {
    const accounts = (this.store!.get('accounts') || []) as Account[]
    const providers = (this.store!.get('providers') || []) as Provider[]
    const normalized = accounts.map(account => normalizeAccountIdentity(
      account, providers.find(provider => provider.id === account.providerId)?.name || account.providerId,
    ))
    if (normalized.some((account, index) => account.name !== accounts[index].name
      || account.nameSource !== accounts[index].nameSource
      || account.email !== accounts[index].email
      || account.providerUserId !== accounts[index].providerUserId)) {
      this.store!.set('accounts', normalized)
    }
  }
  ...
  updateAccount(id: string, updates: Partial<Account>): Account | null {
    ...
    const identity = normalizeAccountIdentity({
      ...accounts[index],
      ...updates,
    }, this.getProviderById(accounts[index].providerId)?.name || accounts[index].providerId)
  ...
  deleteAccount(id: string): boolean {
    ...
    this.store!.set('accounts', accounts.filter(account => account.id !== id))
    this.refreshAccountAvailability()
    this.notifyAccountsChanged()
    return true
  }
  ...
  getActiveAccounts(includeCredentials: boolean = false): Account[] {
    ...
    this.refreshAccountAvailability()
    const accounts = this.store!.get('accounts') as Account[] || []
    const active = accounts.filter((a: Account) => accountAvailability(a).available)
  ...
  getEffectiveModels(providerId: string): EffectiveModel[] {
    ...
    const catalog = providerId === 'arena' ? savedArenaCatalog(provider.supportedModels, provider.modelMappings) : provider
    const defaultModels = catalog.supportedModels || []
    const modelMappings = catalog.modelMappings || {}
    ...
    // Arena aliases come only from the current browser catalog, never arbitrary wildcard overrides.
    ;(providerId === 'arena' ? [] : overrides.addedModels).forEach(customModel => {
      effectiveModels.push({
  ...
  addCustomModel(providerId: string, model: CustomModel): EffectiveModel[] {
    if (providerId === 'arena') throw new Error('Arena models must be discovered from its signed-in browser. Refresh models instead of adding arbitrary IDs.')
    this.ensureInitialized()
    ...
    if (builtinConfig) {
      const providers = (this.store!.get('providers') as Provider[] || []).map(provider => {
        ...
        return {
          ...
          apiEndpoint: builtinConfig.apiEndpoint,
          chatPath: builtinConfig.chatPath,
          ...(providerId === 'arena' ? savedArenaCatalog(provider.supportedModels, provider.modelMappings) : {
            supportedModels: builtinConfig.supportedModels,
            modelMappings: builtinConfig.modelMappings,
          }),
          headers: builtinConfig.headers,
