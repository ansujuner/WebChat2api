...
export const minimaxConfig: BuiltinProviderConfig = {
  ...
  enabled: true,
  description: 'MiniMax Agent web chat (server-selected model); the legacy Matrix route does not select MiniMax M3 or a fixed model version',
  supportedModels: [
    'MiniMax-Agent',
  ],
  modelMappings: {
    'MiniMax-Agent': 'minimax-agent',
    // Compatibility alias only: Matrix send_msg never selected M2.7 by name.
    'MiniMax-M2.7': 'minimax-agent',
  },
  credentialFields: [
    {
      name: 'token',
