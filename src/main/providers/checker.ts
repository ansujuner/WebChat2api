    ...
    switch (provider.id) {
      case 'arena':
        try {
          ...
          return status.authenticated ? { valid: true, userInfo: status.accountInfo } : { valid: false, error: 'Arena sign-in requires attention in its isolated browser.' }
        } catch { return { valid: false, error: 'Use Arena isolated browser login instead of importing tokens.' } }
      case 'deepseek':
      ...
      if (response.status === 200 && response.data?.result?.access_token) {
        return {
          ...
          userInfo: {
            ...
            userId: response.data.result.user?.id || response.data.result.user_id,
          },
        }
      }
