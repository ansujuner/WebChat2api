...
export class TrayManager {
  ...
  public setProxyControls(controls: NonNullable<TrayManager['proxyControls']>): void {
    this.proxyControls = controls
  }
  ...
  private buildMenu(): Menu {
    ...
    const status = this.proxyControls?.getStatus()
    const running = status?.isRunning ?? this.isRunning
    const template: MenuItemConstructorOptions[] = [
      {
        label: isZh
          ? `状态: ${running ? '运行中' : '已停止'}${status ? ` (${status.host}:${status.port})` : ''}`
          : `Status: ${running ? 'Running' : 'Stopped'}${status ? ` (${status.host}:${status.port})` : ''}`,
        enabled: false,
      },
      {
        label: isZh
          ...
          : running
            ...
            : 'Start Proxy',
        click: async () => {
          if (!this.proxyControls) return
          try {
            const current = this.proxyControls.getStatus()
            const success = await (current.isRunning ? this.proxyControls.stop() : this.proxyControls.start())
            if (!success) console.error('Tray proxy operation failed; check application Logs')
          } catch (error) {
            console.error('Tray proxy operation failed:', error)
          }
        },
      },
      { type: 'separator' },
