...
export class ToolCallingEngine {
  ...
  transformRequest(input: {
    ...
    actualModel: string
    requestId?: string
    /** Internal only: the upstream conversation already retains the initial instructions. */
    contextAlreadyInitialized?: boolean
    ...
    const initialPlan = buildToolCallingRuntimePlan({
      requestId,
    ...
    const plan = input.contextAlreadyInitialized
      ? { ...initialPlan, shouldInjectPrompt: false,
        diagnostics: { ...initialPlan.diagnostics, injected: false, reason: 'conversation_instructions_reused' } }
      : initialPlan
