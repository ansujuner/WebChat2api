...
export function unwrapCdata(value: string): string {
  ...
  if (!trimmed.startsWith('<![CDATA[')) return value
  let result = '', offset = 0
  while (offset < trimmed.length) {
    if (!trimmed.startsWith('<![CDATA[', offset)) return value
    const end = trimmed.indexOf(']]>', offset + 9)
    if (end === -1) return value
    result += trimmed.slice(offset + 9, end)
    offset = end + 3
  }
  return result
}
