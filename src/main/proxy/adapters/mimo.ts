...
export function resolveMimoModelConfig(
  request: Pick<ChatCompletionRequest, 'model' | 'originalModel' | 'temperature' | 'reasoning_effort' | 'web_search'>,
) {
  const modelLower = request.model.toLowerCase()
  const isCurrentModel = modelLower === 'mimo-v2.5-pro' || modelLower === 'mimo-v2.5'
  const featureName = (request.originalModel || request.model).toLowerCase()
  // /open-apis/bot/config on 2026-09-06: both V2.5 models default to thinking
  // and automatic search. These are Studio fields, not the paid API schema.
  return {
    enableThinking: request.reasoning_effort === false
      ? false
      : Boolean(request.reasoning_effort) || isCurrentModel || /think|r1/.test(featureName),
    webSearchStatus: (request.web_search ?? (isCurrentModel || featureName.includes('search')))
      ? 'auto'
      : 'disabled',
    model: isCurrentModel ? modelLower : request.model,
...
export class MimoAdapter {
  ...
  async chatCompletion(request: ChatCompletionRequest): Promise<{
    ...
    const conversationId = request.conversation?.sessionId || uuid(false)
    ...
    request.onConversation?.({ ...request.conversation, sessionId: conversationId })
    ...
    const requestBody = {
      ...
      query,
      isEditedQuery: false,
      modelConfig: resolveMimoModelConfig(request),
      multiMedias: [],
    }
...
export class MimoStreamHandler {
  ...
  handleStream(stream: NodeJS.ReadableStream): Readable {
    const output = Readable.from(this.generateStream(stream))
    const destroy = output.destroy.bind(output)
    // Destroy the input before Readable.from waits for the generator's pending
    // next()/return(); waiting for output.close here can deadlock on an idle SSE.
    output.destroy = (error?: Error) => {
      const upstream = stream as Readable
      if (!output.readableEnded && !upstream.readableEnded && !upstream.destroyed) upstream.destroy?.()
      return destroy(error)
    }
    return output
  }

  private async *parseChunks(stream: NodeJS.ReadableStream): AsyncGenerator<MimoChunk> {
    ...
    for await (const chunk of stream) {
      ...
      while ((newline = buffer.indexOf('\n')) !== -1) {
        ...
        if (line.startsWith('event:')) event = line.slice(6).trim()
          ...
          catch { throw new Error('MiMo returned malformed SSE JSON') }
          if (!data || typeof data !== 'object' || Array.isArray(data)) throw new Error('MiMo returned an invalid SSE event')
          if (event === 'error' || data.error || data.success === false || (data.code !== undefined && Number(data.code) !== 0)) {
            throw new Error('MiMo returned an upstream error event')
          }
          if (data.content !== undefined && typeof data.content !== 'string') throw new Error('MiMo returned invalid event content')
          yield { ...data, type: event || data.type } as MimoChunk
        }
      }
    }
    if ((buffer + decoder.end()).trim()) throw new Error('MiMo returned a truncated SSE frame')
  }

  private async *generateStream(stream: NodeJS.ReadableStream): AsyncGenerator<string> {
    const id = `chatcmpl-${uuid(false)}`
    const created = Math.floor(Date.now() / 1000)

    yield this.formatOpenAIChunk(id, created, { role: 'assistant', content: '' })
    ...
    for await (const mimoChunk of this.parseChunks(stream)) {
            if ((mimoChunk.type === 'message' || mimoChunk.type === 'text') && mimoChunk.content) {
              ...
              if (state === 'init') {
                ...
                if (thinkStartIdx !== -1) {
                  // The opening marker may span multiple SSE messages too.
                  const thinkStartEnd = totalContent.indexOf('>', thinkStartIdx)
                  if (thinkStartEnd === -1) continue
                  state = 'thinking'
                  lastProcessedIndex = thinkStartEnd + 1
                } else if ('<think'.startsWith(totalContent.trimStart())) {
                  continue
                } else {
                  state = 'content'
                }
              ...
              if (state === 'thinking') {
                if (!thinkEndTagFound) {
                  ...
                  if (thinkEndIdx !== -1) {
                    ...
                    const cleanedThinkWithCitations = stripCitationsWithBuffer(thinkContent, this.thinkingCitationBuffer)
                    ...
                    // Keep a possible closing-marker prefix for the next SSE
                    // message. Advancing past '</thi' loses the later close
                    // marker and incorrectly sends the answer as reasoning.
                    let safeEndIndex = totalContent.length
                    for (const tag of [thinkEndTag1, thinkEndTag2]) {
                      for (let length = 1; length < tag.length; length++) {
                        if (totalContent.endsWith(tag.slice(0, length))) {
                          safeEndIndex = Math.min(safeEndIndex, totalContent.length - length)
                        }
                      }
                    }
                    const thinkContent = totalContent.slice(lastProcessedIndex, safeEndIndex)
                    const cleanedThinkWithCitations = stripCitationsWithBuffer(thinkContent, this.thinkingCitationBuffer)
                    ...
                    lastProcessedIndex = safeEndIndex
                  }
                }
              }
              ...
              this.onConversation?.({ sessionId: this.conversationId, parentMessageId: this.dialogId })
            } else if (mimoChunk.type === 'finish') {
              completed = true
            }
    }

    if (!completed) throw new Error('MiMo stream ended without a finish event')
