...
import { DEFAULT_GLM_ASSISTANT_ID, resolveGlmModelOptions } from './glm-model-options.ts'
...
export class GLMAdapter {
  ...
  private async uploadFile(fileUrl: string): Promise<{ source_id: string; file_url?: string }> {
    console.log('[GLM] Uploading file')
    ...
    console.log('[GLM] File uploaded successfully')
  ...
  private messagesToPrompt(messages: GLMMessage[], refs: any[] = [], toolsPrompt?: string): { role: string; content: any[] }[] {
    ...
    // Only this turn's new content is supplied by the proxy. Keep a single
    // ordinary user message verbatim instead of adding transcript role markers.
    const parts = processedMessages.map(message => {
      const text = Array.isArray(message.content)
        ? message.content.filter(part => part.type === 'text').map(part => part.text).join('\n')
        : String(message.content || '')
      return message.role === 'system' ? `System: ${text}` : text
    })
    const text = [...parts, ...(toolsPrompt ? [toolsPrompt] : [])].join('\n\n')
    return [{ role: 'user', content: [...content, { type: 'text', text }] }]
  }
...
- Use exact tool names from list, case-sensitive, do not rename.`
    ...
    for (const fileUrl of fileUrls) {
      try {
        ...
        console.error('[GLM] Failed to upload file; remote error details omitted')
      }
    }
    ...
    for (const imageUrl of imageUrls) {
      try {
        ...
        console.error('[GLM] Failed to upload image; remote error details omitted')
      }
    }

    const preparedMessages = this.messagesToPrompt(messages, refs, toolsPrompt)

    const { assistantId, selectedModel, chatMode, isNetworking } = resolveGlmModelOptions(request)
    ...
    const response = await axios.post(
      `${GLM_API_BASE}/backend-api/assistant/stream`,
      {
        ...
        conversation_id: request.conversation?.sessionId || '',
        project_id: '',
        chat_type: 'user_chat',
  ...
  async deleteConversation(conversationId: string): Promise<boolean> {
    try {
      ...
      await axios.post(
        ...
        {
          assistant_id: DEFAULT_GLM_ASSISTANT_ID,
      ...
      console.log('[GLM] Conversation deleted')
      return true
    } catch (error) {
      console.error('[GLM] Failed to delete conversation; remote error details omitted')
      return false
    }
  }
  ...
  async deleteAllChats(): Promise<boolean> {
    try {
      ...
      while (hasMore) {
        ...
        console.log('[GLM] Get conversation list page', page, 'response status:', listResponse.status)
      ...
      console.log('[GLM] Bulk delete response status:', deleteResponse.status)
      ...
      console.error('[GLM] Failed to delete all chats; remote error details omitted')
      return false
    }
  }
...
export class GLMStreamHandler {
  ...
  private observeConversation(result: any): void {
    if (typeof result.conversation_id === 'string' && result.conversation_id) {
      this.conversationId = result.conversation_id
      this.conversationListener?.({ sessionId: this.conversationId })
    }
  }
  ...
  async handleStream(stream: any): Promise<PassThrough> {
    ...
    const parser = createParser({
      onEvent: (event: any) => {
        if (completed || transStream.destroyed) return
        try {
          const result = JSON.parse(event.data)
          if (result.error || result.status === 'error') throw new Error('GLM reported an upstream error')
          ...
          transStream.destroy(err instanceof Error ? err : new Error('Invalid GLM response event'))
        }
      },
    })
    ...
    stream.once('error', (error: Error) => transStream.destroy(error))
    const failIncomplete = () => {
      if (!completed) transStream.destroy(new Error('GLM response ended before its completion marker'))
    }
  ...
  async handleNonStream(stream: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      const failIncomplete = () => {
        if (!completed) reject(new Error('GLM response ended before its completion marker'))
      }
      stream.once('end', failIncomplete)
      stream.once('close', failIncomplete)
    })
