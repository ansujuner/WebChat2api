...
/** get_chat_detail is chronological. Never return an older turn while send_msg is still pending. */
export function selectMiniMaxTurnResponse(messages: any[], requestMessageId: string): any | undefined {
  if (!Array.isArray(messages) || !requestMessageId) return undefined
  const requestIndex = messages.findIndex(message => String(message.msg_id) === String(requestMessageId))
  if (requestIndex < 0) return undefined
  const following = messages.slice(requestIndex + 1)
  const nextUserIndex = following.findIndex(message => message.msg_type === 1)
  const turn = nextUserIndex < 0 ? following : following.slice(0, nextUserIndex)
  return [...turn].reverse().find(message => message.msg_type === 2 &&
    (typeof message.msg_id === 'string' || typeof message.msg_id === 'number') && String(message.msg_id))
}
...
export class MiniMaxAdapter {
  ...
  private messagesPrepare(messages: MiniMaxMessage[], toolsPrompt?: string, isMultiTurn: boolean = false): any {
    if (messages.length === 1 && messages[0].role === 'user' && typeof messages[0].content === 'string' && !toolsPrompt) {
      return { msg_type: 1, text: messages[0].content, chat_type: 1, attachments: [], selected_mcp_tools: [], backend_config: {}, sub_agent_ids: [] }
    }
    ...
    const systemContent = processedMessages.filter(message => message.role === 'system')
      .map(message => typeof message.content === 'string' ? message.content : '')
      .filter(Boolean).join('\n\n')
    const otherMessages = processedMessages.filter(message => message.role !== 'system')
  ...
  async chatCompletion(request: ChatCompletionRequest): Promise<{ response: AxiosResponse | null; stream: { session: ClientHttp2Session; stream: ClientHttp2Stream } | null; chatId: string }> {
    ...
    // Validate before device/auth requests; never relabel this route as M3.
    this.model = resolveMiniMaxWebModel(request.model || 'minimax-agent')
    this.created = unixTimestamp()
    ...
    let msgId: string = ''
    let chatId: string = request.conversation?.sessionId || request.chatId || ''
    ...
    if (!chatId || !msgId) throw new Error('MiniMax send_msg did not return conversation/message IDs')
    chatId = String(chatId)
    msgId = String(msgId)
    request.onConversation?.({ sessionId: chatId, extras: { requestMessageId: msgId } })
    ...
    if (request.stream === true) {
      ...
      const shouldDeleteSession = () => {
        ...
        return !request.retainConversation && config?.mode === 'single' && config?.deleteAfterTimeout
      }
    ...
    const aiMessage = await this.pollForResponse(chatId, deviceInfo, msgId)
    request.onConversation?.({ sessionId: chatId, parentMessageId: String(aiMessage.msg_id), extras: { requestMessageId: msgId } })
  ...
  private async pollForResponse(chatId: string, deviceInfo: DeviceInfo, requestMessageId: string, maxPolls = 120, pollInterval = 1000): Promise<any> {
    let pollCount = 0
    ...
    while (pollCount < maxPolls) {
      ...
      const aiMessage = selectMiniMaxTurnResponse(messages, requestMessageId)
      
      if (aiMessage && aiMessage.msg_content && chat?.chat_status === 2) {
  ...
  private createPollingStream(chatId: string, deviceInfo: DeviceInfo, model: string, requestMessageId: string, onEnd?: (chatId: string) => Promise<void>, onConversation?: ConversationRequestOptions['onConversation'], toolCallingPlan?: import('../toolCalling/types').ToolCallingPlan): PassThrough {
    ...
    const waitForPoll = () => new Promise<void>(resolve => {
      const timer = setTimeout(() => { cancelWait = undefined; resolve() }, pollInterval)
      cancelWait = () => { clearTimeout(timer); cancelWait = undefined; resolve() }
    })
    transStream.once('close', () => {
      cancelWait?.()
      if (!transStream.readableEnded) controller.abort()
    })
    let sentRole = false
    ...
    const poll = async () => {
      try {
        while (pollCount < maxPolls && !transStream.destroyed) {
          await waitForPoll()
          if (transStream.destroyed) return
          pollCount++
          
          const detailResponse = await this.request('POST', '/matrix/api/v1/chat/get_chat_detail', { chat_id: chatId }, deviceInfo, controller.signal)
          if (transStream.destroyed) return
          ...
          if (aiMessage && aiMessage.msg_content) {
            ...
            if (chatStatus === 2 && aiMessage.msg_content) {
              onConversation?.({ sessionId: chatId, parentMessageId: String(aiMessage.msg_id), extras: { requestMessageId } })
        ...
        transStream.destroy(new Error(`MiniMax did not complete the current turn after ${maxPolls} polls`))
      } catch (err) {
        if (transStream.destroyed) return
        console.error('[MiniMax] Polling error:', err)
        transStream.destroy(err instanceof Error ? err : new Error(String(err)))
      }
    }
