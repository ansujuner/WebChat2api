...
const MODEL_ALIASES: Record<string, string> = {
  // The default and exact IDs come from the public web /api/models catalog,
  // not Alibaba Cloud's separate API catalog (verified 2026-09-06).
  qwen: 'qwen3.7-plus',
  qwen3: 'qwen3.7-plus',
...
export class QwenAiAdapter {
  ...
  mapModel(openaiModel: string): string {
    const model = openaiModel.replace(/-(thinking|fast)$/i, '')
    const lowerModel = model.toLowerCase()

    // Explicit mappings must win, including user-added models. Never reinterpret
    // a new upstream ID as a similarly named, older built-in model.
  ...
  async chatCompletion(request: ChatCompletionRequest): Promise<{
    ...
    if (request.messages.some(message => message.role === 'tool' && !message.tool_call_id)) {
      throw new Error('Qwen AI tool results require tool_call_id')
    }
    ...
    if (modelLower.endsWith('-thinking')) {
      forceThinking = true
    } else if (modelLower.endsWith('-fast')) {
      forceThinking = false
    } else if (modelLower.includes('think') || modelLower.includes('r1')) {
      // Auto-enable thinking based on feature aliases, not a separate model ID.
      forceThinking = true
      console.log('[QwenAI] Thinking mode enabled (from model name keyword)')
    }

    const parentId = request.conversation?.parentMessageId || null
    if (request.conversation && !parentId) {
      throw new Error('Qwen AI continuation is missing the previous assistant response ID; start a new conversation')
    }
    const chatId = request.conversation?.sessionId || await this.createChat(modelId, 'OpenAI_API_Chat')
    request.onConversation?.({ sessionId: chatId, ...(parentId ? { parentMessageId: parentId } : {}) })

    const messages = request.messages
    
    // Extract system message and user message
    let systemContent = ''
    const textContent = (content: QwenAiMessage['content']): string => typeof content === 'string'
    ...
    // The forwarder supplies only the new turn when resuming a conversation.
    for (const msg of messages) {
      if (msg.role === 'system') {
        systemContent += (systemContent ? '\n\n' : '') + textContent(msg.content)
      } else if (msg.role === 'user') {
        conversationParts.push(textContent(msg.content))
      } else if (msg.role === 'tool') {
        conversationParts.push(profile().formatToolResult({ toolCallId: msg.tool_call_id!, content: textContent(msg.content) }))
      } else if (msg.tool_calls?.length) {
        conversationParts.push(profile().formatAssistantToolCalls(msg.tool_calls.map(call => ({
          id: call.id, name: call.function.name, arguments: call.function.arguments,
        }))))
      } else {
        conversationParts.push(`Assistant: ${textContent(msg.content)}`)
      }
    }
...
export class QwenAiStreamHandler {
  ...
  private captureConversation(data: any): void {
    // This is the server's response cursor, never our locally generated user
    // fid/childrenIds or a generic SSE envelope ID.
    const responseId = data['response.created']?.response_id
    if (typeof responseId === 'string' && responseId) {
      this.responseId = responseId
    }
    if (this.chatId && this.responseId) {
      this.onConversation?.({ sessionId: this.chatId, parentMessageId: this.responseId })
    }
  }
  ...
  async handleStream(stream: any): Promise<PassThrough> {
    ...
    const parser = createParser({
      onEvent: (event: any) => {
        ...
        try {
          ...
          if (data.error) {
            fail(new Error('Qwen AI upstream returned an error'))
            return
          }
          console.log('[QwenAI] Parsed JSON data keys:', Object.keys(data))

          this.captureConversation(data)
    ...
    stream.once('close', () => {
      ...
      fail(new Error('Qwen AI upstream stream closed before a completion event'))
    })
    stream.once('end', () => fail(new Error('Qwen AI upstream stream ended before a completion event')))
  ...
  async handleNonStream(stream: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      const parser = createParser({
        onEvent: (event: any) => {
          ...
          try {
            ...
            if (parsed.error) {
              rejectOnce(new Error('Qwen AI upstream returned an error'))
              return
            }

            this.captureConversation(parsed)
            data.id = this.responseId || this.chatId || data.id
      ...
      stream.once('close', () => {
        rejectOnce(new Error('Qwen AI upstream stream closed before a completion event'))
      })
      stream.once('end', () => rejectOnce(new Error('Qwen AI upstream stream ended before a completion event')))
    })
  }
