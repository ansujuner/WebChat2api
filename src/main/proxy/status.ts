...
export class ProxyStatusManager {
  ...
  /** Running address comes from the bound listener; stopped address is the next
   * configured start target, not a constructor default or the previous listener. */
  getPublicStatus(configured: Pick<ProxyConfig, 'port' | 'host'>, running = this.isRunning) {
    return {
      isRunning: running,
      port: running ? this.config.port : configured.port,
      host: running ? this.config.host : configured.host,
      uptime: running && this.startTime ? Date.now() - this.startTime : 0,
      connections: running ? this.statistics.activeConnections : 0,
    }
  }
