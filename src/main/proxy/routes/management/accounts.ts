...
router.post('/accounts', managementAuthMiddleware, async (ctx: Context) => {
  try {
    ...
    if (request.name !== undefined && typeof request.name !== 'string') {
      ctx.status = 400
      ctx.body = createErrorResponse('invalid_request', 'Invalid field: name must be a string')
      return
    }
  ...
  try {
    ...
    if (request.enabled !== undefined) {
      if (typeof request.enabled !== 'boolean') {
        ctx.status = 400
        ctx.body = createErrorResponse('invalid_request', 'enabled must be a boolean')
        return
      }
...
router.put('/accounts/:id/enabled', managementAuthMiddleware, async (ctx: Context) => {
  const enabled = (ctx.request.body as { enabled?: unknown } | undefined)?.enabled
  if (typeof enabled !== 'boolean') { ctx.status = 400; ctx.body = createErrorResponse('invalid_request', 'enabled must be a boolean'); return }
  if (!AccountManager.getById(ctx.params.id)) { ctx.status = 404; ctx.body = createErrorResponse('account_not_found', 'Account not found'); return }
  const account = AccountManager.setEnabled(ctx.params.id, enabled)
  ctx.body = createSuccessResponse(account ? maskCredentials(account) : null)
})

router.post('/accounts/:id/clear-suspension', managementAuthMiddleware, async (ctx: Context) => {
  if ((ctx.request.body as { confirmed?: unknown } | undefined)?.confirmed !== true) { ctx.status = 400; ctx.body = createErrorResponse('invalid_request', 'Explicit confirmation is required'); return }
  if (!AccountManager.getById(ctx.params.id)) { ctx.status = 404; ctx.body = createErrorResponse('account_not_found', 'Account not found'); return }
  const account = AccountManager.clearSuspension(ctx.params.id)
  ctx.body = createSuccessResponse(account ? maskCredentials(account) : null)
})
