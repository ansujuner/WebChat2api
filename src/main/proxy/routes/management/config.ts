...
router.put('/', async (ctx: Context) => {
  try {
    ...
    const updatedConfig = await updateNetworkConfiguration(
      updates, () => ConfigManager.get().oauthProxyMode,
      () => ConfigManager.update(updates as Partial<AppConfig>),
    )
...
router.put('/:key', async (ctx: Context) => {
  try {
    ...
    if (!body || typeof body !== 'object' || Array.isArray(body)) {
      ctx.status = 400
      ctx.body = { success: false, error: { code: 'invalid_request', message: 'Request body must be an object containing value' } }
      return
    }
    ...
    const updatedConfig = await updateNetworkConfiguration(
      updates, () => ConfigManager.get().oauthProxyMode, () => ConfigManager.update(updates),
    )
