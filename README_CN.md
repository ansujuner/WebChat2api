...
> 基于 Chat2API 的维护修改版，修改日期：2026-09-06。保留原作者署名及 GPL-3.0-or-later 许可，详见 [NOTICE](NOTICE)、[修改说明](MODIFICATIONS.md) 和 [当前部署文档](docs/local-deployment.md)。下方保留的上游截图与功能介绍不代表所有供应商目前均可用；发布源码不等于已发布安装包。
...
<p align="center">
  Chat2API 将各大模型的官方网页聊天服务转换为 OpenAI 兼容接口。支持 DeepSeek、GLM、Kimi、MiniMax、Qwen、Z.ai 等渠道，可连接 openlcaw、Cline、Roo-Code 等工具。模型权限、使用额度和订阅要求仍由各供应商决定。
</p>
...
| 服务商           | 认证类型          | OAuth | 模型                                                                                                                                                                                                                                            |
| ------------- | ------------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| DeepSeek      | User Token    | 是     | deepseek-v4-flash, deepseek-v4-pro, deepseek-v4-flash-vision-exp |
| GLM           | Refresh Token | 是     | GLM-5.3-Flash, GLM-5.3 |
| Kimi          | JWT Token     | 是     | Kimi-K3, Kimi-K2.6 |
| MiniMax       | JWT Token     | 是     | MiniMax-Agent |
| Mimo          | Cookie        | 是     | MiMo-V2.5-Pro, MiMo-V2.5 |
| Perplexity    | Cookie        | 是     | Best, Auto, Sonar 2, GPT-5.6 Terra, GPT-5.6 Sol, Gemini 3.8 Flash, Claude Sonnet 5, Claude Opus 5, Kimi K3, GLM 5.3, Grok 4.6, Nemotron 3 Ultra |
| Qwen (国内版)    | SSO Ticket    | 是     | Qwen3.7, Qwen3.8-Max, Qwen3.7-Max, Qwen3.6-Flash |
| Qwen AI (国际版) | JWT Token     | 是     | Qwen3.7-Plus, Qwen3.8-Max |
| Z.ai          | JWT Token     | 是     | GLM-5.3-Flash, GLM-5.3, GLM-5.2, GLM-5-Turbo, GLM-5V-Turbo, GLM-4.7；受前端验证码风控限制，暂不可用 |

供应商适配说明和手动添加模型教程见 [docs/providers](docs/providers/README.md)。

模型核对日期：2026-09-06。详见 [逐家官网核对记录](docs/providers/model-audit-2026-09-06.md)。清单和请求构造已核对，不代表登录生成全部通过；Perplexity 指定模型需要相应订阅，MiniMax-Agent 为服务端选模。更新后重启应用同步内置模型，用户自定义模型不会自动删除。

## 📥 安装

### 下载安装

从 [GitHub Releases](https://github.com/ansujuner/WebChat2api/releases) 下载最新版本：
...
- Node.js 22.18+（推荐 Node.js 24 LTS）
- npm
- Git
...
| 组件   | 技术                    |
| ---- | --------------------- |
| 框架   | Electron 44.2.0 / Chromium 152 |
...
在 **关于** 页面检查更新，或从 [GitHub Releases](https://github.com/ansujuner/WebChat2api/releases) 下载最新版本。
