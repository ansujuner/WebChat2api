...
- **Provider-specific checks still apply.** Z.ai may be temporarily unavailable due to frontend captcha risk control; `FRONTEND_CAPTCHA_REQUIRED` requires manual attention on the website, not automatic retries. Perplexity model access depends on your subscription plan; not every model is free.
