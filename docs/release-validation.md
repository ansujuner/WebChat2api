...
## v1.6.6 — Windows login startup and Arena restoration (2026-09-06)

- Final regression run: **1,534 passed, 0 failed, 0 skipped**, with native-browser discovery enabled. Production build and flat renderer/shared type-check passed; isolated production Electron completed **62/62 checks** and quit cleanly. The main-process ES2022 type-check still has the same 16 pre-existing prompt/client-detector/tool-parser diagnostics, not a clean project-wide result.
- A real installed, signature-verified Chrome reproduced the immediate failure: an elevated Electron host spawned a browser which exited normally and disconnected its private pipe in about 100 ms. The ordinary-window control relaunched under the same isolated test profile. This is consistent with [Chromium's Windows automatic de-elevation flow](https://chromium.googlesource.com/chromium/src.git/+/HEAD/chrome/browser/chrome_browser_main_win.cc), rather than a missing browser or website login rejection.
- A real same-user Explorer-desktop launch proved the repair: non-elevated Electron 44.2.0, successful Chrome `Browser.getVersion`, normal browser close/exit and temporary-profile cleanup, **11/11 stages passed**. This test used a new workspace-only profile and `about:blank`, the actual production discovery/argument/pipe code, and no automation, de-elevation-disable or sandbox-disable switches.
- The Windows launcher now uses that existing verified ordinary-user desktop when its caller is elevated. A fixed helper accepts only a one-use nonce, validates the same user/session and medium integrity, and the parent checks the exact launched executable, application argument and creation time. Missing desktop, malformed/stale handoffs and wrong privileges fail closed. **32/32 launcher/restart/runtime-update fixtures passed** with desktop/process operations mocked; no execution-policy override is used.
- Safe login-stage errors now distinguish unavailable profiles, missing browsers, failed startup, disconnected pipes and pages not ready, in both languages. Arena retains ownership after a failed connection or incomplete close and will not relaunch a still-live profile. Page-readiness failure is not represented as proof of sign-out. Deleting an Arena account or provider also closes its owned browser even if re-login was never used in this process.
- **Real deployed result:** v1.6.6 restarted normally with zero forced terminations. Its main process was verified non-elevated at medium integrity. The existing Arena account restored successfully from its original profile; credentials were committed to the same account, revision increased by one and account scheduling controls were preserved. No new account or replacement profile was used. Health and model-list HTTP **200**, configured and actual port **8081**.

**Boundary:** browser startup and verified account restoration do not certify every provider, chat generation, tools or image models. These checks submitted no chats. DeepSeek remains in its saved cooldown and was not retried. The separate real Z.ai restore also passed authenticated website verification and same-account saving, with its scheduling controls retained. Existing provider network choices are retained; no proxy settings are silently changed. Local reports, browser profiles, identities and credentials are not published.
