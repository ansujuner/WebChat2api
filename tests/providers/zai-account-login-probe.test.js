...
test('production smoke drives real account IPC and browser, mocking website responses only', () => {
  ...
  assert.doesNotMatch(fixture, /webRequest\.onBeforeRequest/)
  assert.match(fixture, /revokeOrigins\.push\(allowFixtureOrigin\(session, origin\)\)/)
  assert.match(fixture, /for \(const revoke of revokeOrigins\) revoke\(\)/)
  assert.ok(fixture.indexOf('for (const revoke of revokeOrigins) revoke()') < fixture.indexOf("session.protocol.unhandle('https')"))
